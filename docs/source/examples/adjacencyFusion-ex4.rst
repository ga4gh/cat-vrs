...
.. list-table::
   ...
   * - ``name``
     - FGFR2(hgnc:3689)::v
     - with the FGFR2 gene identifier per the VICC Fusion representation recommendation using the HGNC identifier
   * - ``description``
     - An FGFR2 gene fusion, where the FGFR2 kinase domain is required to remain intact to satisfy the constraints of this categorical variant
     - --
   * - ``aliases``
     - FGFR2::v, FGFR2 fusion
     - included as aliases
   * - ``extensions``
     - --
     - No extensions are included in this example
   * - ``mappings``
     - --
